    ...
    function render() {
        ...
        if (path === '/durtai') {
            ...
            for (let i=0; i<user.liked_chefs.length; i++) {
                const chef = Togooch.find(c => c.chef_id === user.liked_chefs[i]);
                if (chef) createTogoochCard(chef);
            }
            for (let i=0; i<user.liked_foods.length; i++) {
    ...
    export function renderHoolDetail(foodId) {
        ...
        const chef = Togooch.find(c => c.chef_id === food.chef_id);
        if (!chef) {
    ...
    export function renderChefDetail(chefId) {
        const chef = Togooch.find(c => c.chef_id === chefId);
        const mainPage = document.getElementById('main-page');
        mainPage.innerHTML = `
            <div id="container">
            <div id="Togooch-Container"></div>
            ...
            for(let i=0; i<Hool.length; i++){
                if(Hool[i].chef_id === chefId){
                    createHoolCard(Hool[i]);
                }    
    }
        createTogoochCard(chef);
    }
